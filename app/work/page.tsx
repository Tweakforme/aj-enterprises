...
function ProjectCard({ project, index }: { project: typeof projects[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-dark-200/50 border border-white/10 hover:border-primary/40 transition-all duration-500 overflow-hidden flex flex-col h-full"
    >
      <div className="relative w-full bg-dark-300 overflow-hidden">
        <div className="relative bg-dark-300 border-b border-white/10 px-4 py-3 flex items-center gap-2">
          <div className="flex gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
            <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
            <div className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
          </div>
          <div className="flex-1 mx-4 h-6 bg-dark-100/50 rounded flex items-center px-3">
            <span className="text-[10px] text-white/40 truncate">{project.url}</span>
          </div>
        </div>

        <div className="relative w-full aspect-[16/10] bg-dark-100 overflow-hidden">
          <Image
            src={project.image}
            alt={project.title}
            fill
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-dark-100/90 via-dark-100/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          ...
          >
            <div className="px-6 py-3 bg-primary text-dark-100 font-bold text-sm uppercase tracking-wide flex items-center gap-2 hover:bg-accent transition-colors duration-300">
        ...
        <div className="absolute top-16 left-4 px-3 py-1.5 bg-primary/90 backdrop-blur-sm border border-primary">
          <span className="text-xs font-bold text-dark-100 uppercase tracking-wide">
            {project.category}
          </span>
        </div>
      </div>
      ...
      <div className="p-6 sm:p-8 flex-grow flex flex-col">
        <div className="flex-grow">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-xs sm:text-sm text-white/50 hover:text-primary mb-4 transition-colors"
          >
            {project.url}
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          {project.tags.map((tag, i) => (
            <span
              key={i}
              className="px-3 py-1 text-xs bg-dark-300/50 border border-white/10 text-white/70 hover:border-primary/30 transition-colors"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-gradient-to-br from-primary/5 via-transparent to-transparent pointer-events-none transition-opacity duration-500" />
    </motion.div>
  );
}
...
export default function WorkPage() {
  ...
  return (
    <div className="relative bg-dark-100 text-white">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 border border-primary/20 bg-primary/5 rounded-full mb-6 sm:mb-8">
              ...
              <span className="text-xs font-bold text-primary uppercase tracking-wide">Portfolio</span>
            </div>

            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-6 sm:mb-8 leading-[1.1] tracking-tight">
              <span className="text-white block mb-2">Exceptional Work</span>
              <span className="block bg-clip-text text-transparent bg-gradient-to-r from-primary via-emerald-300 to-yellow-300">
                Built for Growth
              </span>
            </h1>

            <p className="text-base sm:text-lg lg:text-xl text-white/60 max-w-3xl mx-auto leading-relaxed mb-10 sm:mb-12">
      ...
      {/* STATS SECTION */}
      <section ref={statsRef} className="relative py-12 sm:py-16 lg:py-20 border-y border-white/10">
      ...
      <section id="projects" className="relative py-16 sm:py-20 lg:py-24 xl:py-28 overflow-hidden">
        ...
        <div className="pointer-events-none absolute top-1/4 right-1/4 w-[30rem] sm:w-[40rem] h-[30rem] sm:h-[40rem] bg-primary/5 blur-3xl" />
      ...
      <section ref={capabilitiesRef} className="relative py-16 sm:py-20 lg:py-24 overflow-hidden bg-dark-200/30">
        <div className="pointer-events-none absolute top-0 left-1/4 w-[30rem] sm:w-[40rem] h-[30rem] sm:h-[40rem] bg-yellow-300/5 blur-3xl" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {capabilities.map((capability, i) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: i * 0.1 }}
                className="relative group bg-dark-200/50 border border-white/10 hover:border-primary/40 p-6 sm:p-8 transition-all duration-500"
              >
                ...
                <div className="relative">
                  <div className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 mb-4 sm:mb-6 border border-white/10 bg-dark-300 group-hover:border-primary/50 transition-all duration-300">
                    <capability.icon className="w-6 h-6 sm:w-7 sm:h-7 text-primary group-hover:scale-110 transition-transform duration-300" />
                  </div>
                  ...
                  <div className="mt-3 sm:mt-4 h-[2px] w-0 bg-gradient-to-r from-primary via-emerald-300 to-yellow-300 group-hover:w-12 transition-all duration-500" />
                </div>
              </motion.div>
            ))}
          </div>

...
const coreServices = [
  {
    ...
    description: "Perfect for small businesses, professionals, and service providers who need a fast, professional online presence.",
    priceRange: "$800 - $2,500",
    timeline: "5-10 days",
    icon: Globe,
    color: "cyan",
    features: [
      { text: "4-6 Custom Pages", included: true },
      ...
      { text: "Performance Optimization (90+ Lighthouse)", included: true },
  ...
  },
  {
    ...
    description: "Full-featured online stores built on Shopify or custom platforms. Everything you need to run a profitable e-commerce business.",
    ...
    popular: true,
    features: [
      { text: "Custom Store Design", included: true },
      { text: "Product Page Templates", included: true },
      ...
      { text: "Email Notifications & Receipts", included: true },
      { text: "Shipping Calculator Integration", included: true },
      { text: "Abandoned Cart Recovery", included: true },
      { text: "Product Reviews & Ratings", included: true },
      { text: "Mobile Shopping Optimization", included: true },
      { text: "Advanced SEO for E-Commerce", included: true },
      { text: "1 Year Free Maintenance", included: true },
  ...
  },
  {
    ...
    description: "Complex web applications, multi-vendor marketplaces, SaaS platforms, and custom integrations. Built for growth.",
    priceRange: "$6,000+",
    timeline: "3-8 weeks",
    icon: Cpu,
    color: "gradient",
    features: [
      ...
      { text: "User Authentication & Authorization", included: true },
      { text: "Admin Dashboard & CMS", included: true },
      { text: "API Development & Integrations", included: true },
      { text: "Third-Party Service Connections", included: true },
      { text: "Advanced Data Analytics", included: true },
      { text: "Multi-User Role Management", included: true },
      { text: "Real-Time Features & Websockets", included: true },
      { text: "Cloud Infrastructure Setup", included: true },
      { text: "Performance & Security Audits", included: true },
      { text: "Load Balancing & Scaling", included: true },
      { text: "1 Year Free Maintenance", included: true },
...
const specializedServices = [
  ...
  },
  {
    ...
    capabilities: [
      ...
      "CRM & Email Platform Connections",
...
// Add-ons
const addOns = [
  { name: "Blog Setup & CMS", price: "$400" },
  { name: "Member Portal / Login System", price: "$800" },
  { name: "Booking/Scheduling System", price: "$600" },
  { name: "Live Chat Integration", price: "$300" },
  { name: "Email Marketing Setup", price: "$400" },
  { name: "Social Media Integration", price: "$300" },
  { name: "Multi-Language Support", price: "$800" },
  { name: "Advanced Analytics Dashboard", price: "$700" },
  { name: "Custom Email Templates", price: "$400" },
  { name: "SSL & Security Hardening", price: "$300" },
];
...
function ServiceCard({ service, index }: { service: typeof coreServices[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative group ${service.popular ? 'lg:-mt-6 lg:mb-6' : ''}`}
    >
      {/* Popular Badge */}
      {service.popular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-20 px-4 py-1.5 bg-gradient-to-r from-primary-light to-accent-light dark:from-primary dark:to-accent rounded-full">
          <span className="text-xs font-bold text-white dark:text-dark-100 uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div className={`relative h-full bg-white dark:bg-dark-200 border-2 ${
        service.popular 
          ? 'border-primary-light dark:border-primary shadow-xl dark:shadow-primary/20' 
          : 'border-ocean-200 dark:border-white/10'
      } rounded-2xl p-8 transition-all duration-500 hover:border-primary-light dark:hover:border-primary group-hover:shadow-2xl overflow-hidden`}>
        
        {/* Background Glow */}
        <div className={`absolute top-0 right-0 w-64 h-64 ${
          service.color === 'cyan' ? 'bg-primary-light/5 dark:bg-primary/5' :
          service.color === 'yellow' ? 'bg-accent-light/5 dark:bg-accent/5' :
          'bg-gradient-to-br from-primary-light/5 to-accent-light/5 dark:from-primary/5 dark:to-accent/5'
        } blur-3xl rounded-full transition-all duration-500 group-hover:scale-150`} />

        {/* Header */}
        <div className="relative z-10 mb-8">
          <div className="flex items-start justify-between mb-4">
            <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${
              service.color === 'cyan' ? 'bg-primary-light/10 dark:bg-primary/10' :
              service.color === 'yellow' ? 'bg-accent-light/10 dark:bg-accent/10' :
              'bg-gradient-to-br from-primary-light/10 to-accent-light/10 dark:from-primary/10 dark:to-accent/10'
            }`}>
              <Icon className={`w-7 h-7 ${
                service.color === 'cyan' ? 'text-primary-light dark:text-primary' :
                service.color === 'yellow' ? 'text-accent-light dark:text-accent' :
                'text-primary-light dark:text-primary'
              }`} />
            </div>
            
            <div className="text-right">
              <div className="text-xs font-bold text-ocean-500 dark:text-white/40 uppercase tracking-wider mb-1">
                {service.tier}
              </div>
              <div className="text-sm text-ocean-600 dark:text-white/50">
                {service.timeline}
              </div>
            </div>
          </div>

          <h3 className="font-display text-2xl sm:text-3xl font-bold mb-2 text-ocean-900 dark:text-white">
            {service.name}
          </h3>
          
          <p className="text-sm font-medium bg-clip-text text-transparent bg-gradient-to-r from-primary-light to-accent-light dark:from-primary dark:to-accent mb-3">
            {service.tagline}
          </p>

          <p className="text-sm text-ocean-600 dark:text-white/60 leading-relaxed mb-4">
            {service.description}
          </p>
          ...
          <div className="flex items-baseline gap-2">
            ...
            {service.priceRange.includes('-') && (
              <span className="text-lg text-ocean-600 dark:text-white/60">
                {service.priceRange.split(' ')[1]} {service.priceRange.split(' ')[2]}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="relative z-10 space-y-3 mb-8">
          {service.features.map((feature, idx) => (
            <div key={idx} className="flex items-start gap-3">
              <CheckCircle2 className="w-5 h-5 text-primary-light dark:text-primary flex-shrink-0 mt-0.5" />
              <span className="text-sm text-ocean-700 dark:text-white/70">
                {feature.text}
              </span>
            </div>
          ))}
        ...
        {/* Ideal For */}
        <div className="relative z-10 mb-8">
          <div className="text-xs font-bold text-ocean-500 dark:text-white/40 uppercase tracking-wider mb-3">
            Ideal For
          </div>
          <div className="flex flex-wrap gap-2">
            {service.ideal.map((item, idx) => (
              <span
                key={idx}
                className="px-3 py-1 bg-ocean-100 dark:bg-dark-300 text-xs text-ocean-700 dark:text-white/70 rounded-full border border-ocean-200 dark:border-white/10"
              >
                {item}
              </span>
            ))}
        ...
        {/* CTA */}
        <Link href="/contact">
          <button className="relative z-10 w-full py-4 bg-primary-light dark:bg-primary text-white dark:text-dark-100 font-bold text-sm uppercase tracking-wide rounded-xl hover:bg-accent-light dark:hover:bg-accent transition-all duration-300 flex items-center justify-center gap-2 group/btn">
...
function SpecializedServiceCard({ service, index }: { service: typeof specializedServices[0]; index: number }) {
  ...
  return (
    ...
    >
      <div className="relative h-full bg-white dark:bg-dark-200 border border-ocean-200 dark:border-white/10 rounded-xl p-6 transition-all duration-300 hover:border-primary-light dark:hover:border-primary hover:shadow-lg overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between mb-4">
            <div className="w-12 h-12 rounded-lg bg-primary-light/10 dark:bg-primary/10 flex items-center justify-center">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link href="/contact">
                <button className="px-8 py-4 bg-primary-light dark:bg-primary text-white dark:text-dark-100 font-bold text-sm uppercase tracking-wide rounded-xl hover:bg-accent-light dark:hover:bg-accent transition-all duration-300 flex items-center gap-2 group">
              ...
              <a href="#process">
                <button className="px-8 py-4 border-2 border-ocean-200 dark:border-white/20 text-ocean-900 dark:text-white font-semibold text-sm uppercase tracking-wide rounded-xl hover:border-primary-light dark:hover:border-primary hover:bg-primary-light/10 dark:hover:bg-primary/10 transition-all duration-300">
                  See How It Works
                </button>
              </a>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
            {platforms.map((platform, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-light-100 dark:bg-dark-200 border border-ocean-200 dark:border-white/10 rounded-xl p-6 text-center hover:border-primary-light dark:hover:border-primary transition-all duration-300 hover:shadow-lg"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
            {addOns.map((addon, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="flex items-center justify-between bg-white dark:bg-dark-100 border border-ocean-200 dark:border-white/10 rounded-lg px-6 py-4 hover:border-primary-light dark:hover:border-primary transition-all duration-300"
              >
              ...
              >
                ...
                <div className="flex gap-6 items-start">
                  <div className="flex-shrink-0">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary-light to-accent-light dark:from-primary dark:to-accent flex items-center justify-center">
                  ...
                  <div className="flex-1 bg-white dark:bg-dark-200 border border-ocean-200 dark:border-white/10 rounded-xl p-6">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                <Link href="/contact">
                  <button className="px-10 py-5 bg-white dark:bg-dark-100 text-primary-light dark:text-primary font-bold text-base uppercase tracking-wide rounded-xl hover:scale-105 transition-all duration-300 shadow-2xl">
                    Get Your Free Quote
                  </button>
                </Link>

                <Link href="/work">
                  <button className="px-10 py-5 border-2 border-white/30 text-white dark:text-dark-100 font-semibold text-base uppercase tracking-wide rounded-xl hover:bg-white/10 hover:border-white/50 transition-all duration-300">
                    View Our Work
                  </button>
                </Link>
              </div>
